function [N_S_ped_on, E_W_ped_on] = EW_ped_red(enable, ljHandle, lj_dac, lj_dig, desired_timing)
    ...
    if nargin > 3
        ...
        for i = 1:desired_timing
            ...
            while toc(tStart) < 1.0
                ...
                if toc(tStart) < 0.5
                    Error = ljud_ePut(ljHandle, lj_dac, 1, write_voltage, 0);
                    Error_Message(Error)
                else
                    Error = ljud_ePut(ljHandle, lj_dac, 1, 0, 0);
                    Error_Message(Error)
                end
